...
export default function Password<T extends FieldValues>({
  ...
  return (
    <div className="w-full flex flex-col">
      <label
        htmlFor={name}
        className="text-white font-bold text-lg mb-3 text-left"
      >
        {label}
      </label>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full pl-5 pr-11 py-4 rounded-[2px] text-white font-light text-base outline ${
            fieldState.error
              ? "outline-red"
              : "outline-gray-200 focus:outline-main"
          } focus:outline-2`}
        />
        <button
          type="button"
        ...
        >
          <Image
            src={
              isPasswordVisible
                ? ICON_PATHS.iconVisible
                : ICON_PATHS.iconInvisible
            } // 아이콘 변경
            alt="비밀번호 미리보기"
            width={24}
            height={24}
      ...
      {fieldState.error && (
        <p className="text-red text-base font-light mt-1 text-left">
