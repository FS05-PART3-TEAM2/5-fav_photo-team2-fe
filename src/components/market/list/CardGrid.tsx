...
const PhotoCardList = ({ photoCards, isFetching, observerRef }: PhotoCardListProps) => {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 gap-[5px] sm:gap-[20px] lg:gap-[40px]">
      {Array.isArray(photoCards) && photoCards.length === 0 ? (
        <div className="col-span-2 lg:col-span-3 text-center text-gray-400 text-lg h-[400px]">
          해당하는 포토카드를 불러오는 중이거나 해당하는 포토카드가 없습니다.
        </div>
      ) : (
        photoCards.map((card, index) => {
          ...
          return (
            <Link href={`/market/${card.saleCardId}`} key={card.saleCardId}>
              <div ref={isLast ? observerRef : undefined}>{cardElement}</div>
            </Link>
          );
        })
      )}
